...
// Stripe Checkout redirect form — no card input on our side; Stripe hosts the payment page
const RentalPaymentForm = ({ property }) => {
  ...
  useEffect(() => {
    ...
    endDate.setMonth(endDate.getMonth() + 1); // default 1 month on mount
  ...
  const handleSubmit = async (event) => {
    ...
    if (!termsAccepted) {
      setError('Vui lòng đồng ý với các điều khoản và điều kiện thuê nhà');
      return;
    }
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!transactionId) throw new Error('Không nhận được ID giao dịch');
      ...
      toast.error(err.message || 'Không thể khởi tạo thanh toán. Vui lòng thử lại sau.');
      setError(err.message || 'Không thể khởi tạo thanh toán. Vui lòng thử lại sau.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="mt-4 space-y-4">
      ...
      <div className="bg-gray-50 p-4 rounded-md">
        ...
        <div className="flex justify-between mb-2">
          ...
          <span className="text-sm font-medium">{formatVND(property.rentPrice || property.price)}</span>
        </div>
        <div className="flex justify-between mb-2">
        ...
        <div className="flex justify-between font-medium">
          ...
          <span className="text-rose-600">{formatVND(calculateTotalAmount())}</span>
        </div>
      </div>

      <div className="rounded-md bg-blue-50 border border-blue-200 p-3 flex items-center gap-2 text-sm text-blue-800">
        <svg className="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
        </svg>
        Bạn sẽ được chuyển đến trang thanh toán bảo mật của Stripe để hoàn tất giao dịch.
      </div>
      ...
      <div className="flex items-start">
        ...
        <label htmlFor="terms" className="ml-3 text-sm font-medium text-gray-700">
          Tôi đồng ý với{' '}
          <a href="#" className="text-rose-600 hover:text-rose-500">điều khoản và điều kiện thuê nhà</a>
        </label>
      </div>
      ...
      <button
        ...
        className={`w-full flex items-center justify-center gap-2 bg-rose-600 hover:bg-rose-700 text-white font-bold py-3 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-500 ${(loading || !termsAccepted) ? 'opacity-70 cursor-not-allowed' : ''}`}
      >
        {loading ? (
          <>
          ...
          `Thanh toán ${formatVND(calculateTotalAmount())} qua Stripe`
        )}
      </button>
    </form>
  );
