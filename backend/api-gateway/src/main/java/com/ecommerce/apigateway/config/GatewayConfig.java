...
public class GatewayConfig {
    ...
    public RouteLocator customRouteLocator(RouteLocatorBuilder builder) {
        ...
        return builder.routes()
                ...
                // OAuth2 Routes - No JWT filter (browser redirects, provider callbacks)
                .route("auth-service-oauth2", r -> r
                        .path("/api/auth/oauth2/**")
                        .filters(f -> {
                            logger.debug("OAuth2 route matched");
                            return f;
                        })
                        .uri("lb://auth-service"))
                ...
                // Category Routes - served by property-service (uses PropertyCategory enum)
                .route("category-service-public", r -> r
                        .path("/api/categories/public/**")
                        .filters(f -> {
                            logger.debug("Category public route matched - routing to property-service");
                            return f;
                        })
                        .uri("lb://property-service"))
                ...
                .route("category-service-protected", r -> r
                        .path("/api/categories/**")
                        .filters(f -> {
                            logger.debug("Category protected route matched - routing to property-service");
                            JwtAuthenticationFilter.Config config = new JwtAuthenticationFilter.Config();
                            return f.filter(jwtAuthenticationFilter.apply(config));
                        })
                        .uri("lb://property-service"))

                // Transaction Service Routes - Public Webhook endpoints (no JWT)
                ...
                // Stripe Checkout Session Route - Protected
                .route("stripe-session-protected", r -> r
                        .path("/api/stripe/**")
                        .filters(f -> {
                            logger.debug("Stripe session route matched - routing to transaction-service");
                            JwtAuthenticationFilter.Config config = new JwtAuthenticationFilter.Config();
                            return f.filter(jwtAuthenticationFilter.apply(config));
                        })
                        .uri("lb://transaction-service"))
